...
int RBFTest_5(PagedFileManager *pfm)
{
    ...
    assert(rc == success);
    fileHandle.getQFile()->unmap((uchar*)buffer);
...
int RBFTest_8(RecordBasedFileManager *rbfm) {
    ...
    cout << "Data instered." << endl;

    rc = rbfm->insertRecord(fileHandle, recordDescriptor, record, rid);
    assert(rc == success);
    cout << "Row instered." << endl;
...
int RBFTest_9(RecordBasedFileManager *rbfm, vector<RID> &rids, vector<int> &sizes) {
    ...
    for(int i = 0; i < numRecords; i++)
    {
        ...
        cout << "Record is supposed to be:\n" << (char*)record << endl;
        rc = rbfm->insertRecord(fileHandle, recordDescriptor, record, rid);
        cout << rc << endl;
        assert(rc == success);

        // START TEMP DEBUG - test if inserted record was actually inserted.
        memset(record, 0, 1000);
        rc = rbfm->readRecord(fileHandle, recordDescriptor, rid, record);
        cout << rc << endl;
        assert(rc == success);
        cout << "Record is actually:\n" << (char*)record << "\n\n\n";
        // END TEMP DEBUG
